...
public class MypageDAO {
	...
	public ArrayList<TempBean> getTempProjectList(String u_idx) {
		...
		try {
			con = pool.getConnection();
			sql = "select * from temp_TB where tp_u_idx = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, u_idx);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public ArrayList<FundBean> getSearchFundBox(String u_idx, String pj_title) {
		...
		try {
			...
			con = pool.getConnection();
			// 펀드 table에서 후원자 Idx 일치하는 행
			sql = "select FT.f_pj_idx, FT.f_dt , FT.f_price , FT.f_idx , PET.p_explain ,PT.obj_price, PT.pj_f_price, PT.pj_title, PT.pj_img from PJ_TB PT Inner Join Fund_TB FT on PT.pj_idx = FT.f_pj_idx Inner Join PRESENT_TB PET on FT.f_p_idx = PET.p_idx where FT.supmem_idx = ? and PT.pj_title like ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, u_idx);
