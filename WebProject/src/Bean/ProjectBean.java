package Bean;

public class ProjectBean {
	private int pj_idx;
	private String pj_title;
	private String pj_short_title;
	private String pj_img;
	private String pj_summary;
	private String pj_category;
	private String pj_url;
	private String search_tag;
	private String profile_img;
	private String leader_nm;
	private String leader_intro;
	private String leader_loc;
	private String pj_end_dt;
	private String pj_intro_video;
	private int pj_sup_num;
	private int pj_u_idx;
	private String pj_f_price;
	private String pj_story;
	private String obj_price;
	private String pj_reg_dt;
	private String ac_no;
	private String ah;
	private String bank_nm;
	private String ah_birth;
	
	
	public String getAc_no() {
		return ac_no;
	}
	public void setAc_no(String ac_no) {
		this.ac_no = ac_no;
	}
	public String getAh() {
		return ah;
	}
	public void setAh(String ah) {
		this.ah = ah;
	}
	public String getBank_nm() {
		return bank_nm;
	}
	public void setBank_nm(String bank_nm) {
		this.bank_nm = bank_nm;
	}
	public String getAh_birth() {
		return ah_birth;
	}
	public void setAh_birth(String ah_birth) {
		this.ah_birth = ah_birth;
	}
	public int getPj_idx() {
		return pj_idx;
	}
	public void setPj_idx(int pj_idx) {
		this.pj_idx = pj_idx;
	}
	public String getPj_title() {
		return pj_title;
	}
	public void setPj_title(String pj_title) {
		this.pj_title = pj_title;
	}
	public String getPj_short_title() {
		return pj_short_title;
	}
	public void setPj_short_title(String pj_short_title) {
		this.pj_short_title = pj_short_title;
	}
	public String getPj_img() {
		return pj_img;
	}
	public void setPj_img(String pj_img) {
		this.pj_img = pj_img;
	}
	public String getPj_summary() {
		return pj_summary;
	}
	public void setPj_summary(String pj_summary) {
		this.pj_summary = pj_summary;
	}
	public String getPj_category() {
		return pj_category;
	}
	public void setPj_category(String pj_category) {
		this.pj_category = pj_category;
	}
	public String getPj_url() {
		return pj_url;
	}
	public void setPj_url(String pj_url) {
		this.pj_url = pj_url;
	}
	public String getSearch_tag() {
		return search_tag;
	}
	public void setSearch_tag(String search_tag) {
		this.search_tag = search_tag;
	}
	public String getProfile_img() {
		return profile_img;
	}
	public void setProfile_img(String profile_img) {
		this.profile_img = profile_img;
	}
	public String getLeader_nm() {
		return leader_nm;
	}
	public void setLeader_nm(String leader_nm) {
		this.leader_nm = leader_nm;
	}
	public String getLeader_intro() {
		return leader_intro;
	}
	public void setLeader_intro(String leader_intro) {
		this.leader_intro = leader_intro;
	}
	public String getLeader_loc() {
		return leader_loc;
	}
	public void setLeader_loc(String leader_loc) {
		this.leader_loc = leader_loc;
	}
	public String getPj_end_dt() {
		return pj_end_dt;
	}
	public void setPj_end_dt(String pj_end_dt) {
		this.pj_end_dt = pj_end_dt;
	}
	public String getPj_intro_video() {
		return pj_intro_video;
	}
	public void setPj_intro_video(String pj_intro_video) {
		this.pj_intro_video = pj_intro_video;
	}
	public int getPj_sup_num() {
		return pj_sup_num;
	}
	public void setPj_sup_num(int pj_sup_num) {
		this.pj_sup_num = pj_sup_num;
	}
	public int getPj_u_idx() {
		return pj_u_idx;
	}
	public void setPj_u_idx(int pj_u_idx) {
		this.pj_u_idx = pj_u_idx;
	}
	public String getPj_f_price() {
		return pj_f_price;
	}
	public void setPj_f_price(String pj_f_price) {
		this.pj_f_price = pj_f_price;
	}
	public String getPj_story() {
		return pj_story;
	}
	public void setPj_story(String pj_story) {
		this.pj_story = pj_story;
	}
	public String getObj_price() {
		return obj_price;
	}
	public void setObj_price(String obj_price) {
		this.obj_price = obj_price;
	}
	public String getPj_reg_dt() {
		return pj_reg_dt;
	}
	public void setPj_reg_dt(String pj_reg_dt) {
		this.pj_reg_dt = pj_reg_dt;
	}
}
